...
namespace System.Data
{
    ...
    public static class SqliteRecoveryHelper
    {
        ...
        public static string DataRecovery(string sourcedb, string charatorPath, string tableNames, bool isScanDebris = false)
        {
            ...
            try
            {
                ...
                if (File.Exists(newfile))
                {
                    ...
                    {
                        newfile = System.Utility.Helper.File.ConnectPath(path, string.Format("{0}_recovery_{1}.{2}", name.TrimEnd(ext).TrimEnd("."), DateTime.Now.Second, ext)).TrimEnd(".");
                    }
                }
        ...
        /// <param name="isTryID">是否恢复主键</param>
        /// <returns>恢复成功，则返回新数据库db文件。(极端)失败则返回源来的数据库db文件。</returns>
        public static string DataRecoveryForSPF(string sourcedb, string charatorPath, string tableNames,
            string baseDbPath, string baseCharatorPath, bool isScanDebris = false, Dictionary<string, string> index = null, bool isTryID = false)
        {
            if(!System.Utility.Helper.File.IsValid(sourcedb))
            {
                LogHelper.Error(string.Format("Sqlite恢复文件【{0}】不存在，或者文件大小为0，无法处理。", sourcedb));
            ...
            try
            {
                ...
                //底层C++处理
                var res = ButtomDataRecoveryForSPF(sourcedb, charatorPath, newfile, tableArray, baseDbPath, baseCharatorPath, index, isTryID);
        ...
        public static void DeepScanMirror(string mirrorPath, string baseDbFile, string baseCharatorPath, string tableNames, string newDbPath)
        {
            ...
            if (InitDb(baseDbFile, baseCharatorPath, ref tempDbBase))
            {
                foreach (var tableName in tableNames.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries))
                {
                    ...
                    if (0 == res)
                    {
                        ...
                        {
                            LogHelper.Error(string.Format("特征扫描后获取表定义失败,SOURCE=[{0}] [{1}]", baseDbFile, tableName));
                        }
                    ...
                    {
                        LogHelper.Error(string.Format("特征扫描失败,SOURCE=[{0}] [{1}],错误码 {2}", baseDbFile, tableName, res));
                    }
                }
            }
        }
        ...
        /// 注意若是多个表进行同时处理，只要有一个表处理成功，IsSucess= true。
        /// 其他异常错误信息，可从StackMsg属性中获取概要信息；出错时查看日志文件，可得到更多栈信息。</returns>
        private static SqliteReturn ButtomDataRecoveryForSPF(string sourceDb, string charatorPath, string newDbPath, string[] tableNames, string baseDbPath, string baseCharatorPath, Dictionary<string, string> index = null, bool isTryID = false)
        {
            var sqliteReturn = new SqliteReturn { IsSucess = false, StackMsg = string.Empty };
            if (sourceDb.IsNullOrEmpty() || newDbPath.IsNullOrEmpty() || tableNames.Length <= 0)
            {
            ...
            foreach (var tableName in tableNames)
            {
                ...
                if (isInit == false)
                {
                    if (InitDb(sourceDb, charatorPath, ref dbBase, isTryID))
                    {
                        isInit = true;
                    }
                    else
                ...
                if (allColumnNames.Count == 0)
                {
                    ...
                    {
                        //做文件扫描
                        LogHelper.Info(string.Format("开始特征扫描,SOURCE=[{0}] [{1}]", sourceDb, tableName));
                        ...
                        if (InitDb(baseDbPath, baseCharatorPath, ref tempDbBase))
                        {
                            ...
                            if (0 == res)
                            {
                                ...
                                if (0 == allColumnNames.Count)
                                {
                                    LogHelper.Error(string.Format("特征扫描后获取表定义失败,SOURCE=[{0}] [{1}]", sourceDb, tableName));
                                }
                            ...
                            {
                                LogHelper.Error(string.Format("特征扫描失败,SOURCE=[{0}] [{1}],错误码 {2}", sourceDb, tableName, res));
                            }
                        }
                    }
                    else
        ...
        private static SqliteContext curContext = null;
        private static SQLiteCommand curCommand = null;
        private static bool bCreateTable = false;
        private static string sCurTableName = "";
        private static string sCurIndexName = "";
        private static IList<string> allColumnNames;
        private static IList<string> allColumnTypes;
        ...
        public static int SqliteCallBackTTTT(int count, IntPtr objectPointer, byte dataType)
        {
        ...
        /// 创建表构架。
        /// </summary>
        private static bool CreateTable(SqliteContext context, string tableName, IList<string> allColumnNames, IList<string> allColumnTypes, string indexCol = null)
        {
        ...
        private static bool InitDb(string sourceDb, string charatorPath, ref IntPtr dbBase, bool isTryID = false)
        {
            try
            {
                ...
                if (isTryID)
                {
                    SqliteCoreDll.IsTryID(dbBase, 1);
                }
        ...
        /// <returns></returns>
        public static int Sqlite3_General_OtherScanCallBack(int pagesize, int curpage, int allPage, int scanedCount, ref int stop)
        {
            return 0;
        }
